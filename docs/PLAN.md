...
1. **Foundations**
   ...
   - Build shared utilities: HTTP client (`httpx`), retry + rate limiter, config directory (`~/.config/goodreads-tools`), and structured logging.
...
4. **Automation / agent hooks**
   - Provide a thin Python API (`goodreads_tools.client`) so agent frameworks can script actions.
...
1. `gr login`:
   ...
   - Store cookies + metadata in `~/.config/goodreads-tools/session.json` (consider keyring for encryption).
...
- `goodreads_tools/http.py`: wrappers around `httpx.AsyncClient`, throttle, caching, error translation.
- `goodreads_tools/parsers/search.py`: parse autocomplete JSON.
- `goodreads_tools/parsers/book.py`: load `__NEXT_DATA__` and map to dataclasses.
- `goodreads_tools/parsers/shelf.py`: RSS + HTML parser (use `selectolax` for speed).
- `goodreads_tools/auth.py`: cookie management + CSRF extraction.
...
- Expose a Python API surface (`from goodreads_tools import Client`) that the CLI and agents share.
...
2. Wire `typer` into `src/goodreads_tools/__init__.py`, add `cli.py`, and create commands.
